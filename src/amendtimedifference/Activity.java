...
class Activity {
	...
	private PackageData getRecord(int TAG,String[]record) {
		...
		PackageData aa = new PackageData(TAG,SN,QN,0,TIME);
		return aa;
	}
	 ...
	 */
	protected void getACTlist(ArrayList<PackageData>aat,String[]arrACT,ArrayList<String[]>csvlist) {
		String SN = arrACT[0];
		...
		for(int i=0;i<csvlist.size();i++) {
			...
			if(SN.equals(snlist)&&QN==qnlist) {
				PackageData pd = getRecord(0,eachlist);
				aat.add(pd);
